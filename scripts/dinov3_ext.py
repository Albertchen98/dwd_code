...
def process_video(video_path, output_dir, processors):
    ...
    frames = np.concatenate((np.zeros((3,H,W,C)), frames), axis=0)
    
    # Split frames into batches
    frame_batches = [
        frames[i:i+BATCHSIZE] 
        for i in range(0, n_frames+3, BATCHSIZE)
    ]
    ...
    # features_tranf = rearrange(features_tranf, "t h w c -> c t h w")
    # features_tranf = features_tranf.bfloat16().contiguous()
    save_file({"dino":features}, output_file_path)


@click.command()
@click.option("--input_path", '-i', type=str, help="video path")
...
def main(input_path, output_dir):
    ...
    # Process each video
    process_video(input_path, output_dir, processors)
