...
from cosmos_transfer2._src.predict2.models.text2world_model_rectified_flow import Text2WorldModelRectifiedFlow, Text2WorldModelRectifiedFlowConfig
...
FSDP_TEXT2WORLD_RECTIFIED_FLOW = dict(
    trainer=dict(
        distributed_parallelism="fsdp",
    ),
    model=L(Text2WorldModelRectifiedFlow)(
        config=Text2WorldModelRectifiedFlowConfig(
            fsdp_shard_size=8,
        ),
        _recursive_=False,
    ),
...
def register_model():
    ...
    cs.store(group="model", package="_global_", name="fsdp_text2world_rectified_flow", node=FSDP_TEXT2WORLD_RECTIFIED_FLOW)
