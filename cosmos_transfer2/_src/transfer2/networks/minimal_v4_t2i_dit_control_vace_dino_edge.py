...
class DINOPatchEmbed(nn.Module):
    ...
    def init_weights(self) -> None:
        std =  1.0 / math.sqrt(self.proj[1].out_features)
...
class DINOProjector(nn.Module):
    ...
    def forward(self, dino_latent: torch.Tensor=None, edge_latent: torch.Tensor=None):
        ...
        if self.dist_align:
            dino_latent = match_distribution(dino_latent, edge_latent)
...
class DINOProjectorV2(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.dist_align = dist_align
        self.layer1 = nn.Linear(in_channels, in_channels, bias=bias)
        self.layer2 = nn.Linear(in_channels, out_channels, bias=bias)
        if self.training and random_drop:
            self.drop_rate = 0.2
        else:
            self.drop_rate = 0.0
    ...
    def init_weights(self) -> None:
        std = 1.0 / math.sqrt(self.layer1.out_features)
        torch.nn.init.trunc_normal_(self.layer1.weight, std=std, a=-3 * std, b=3 * std)
        std = 1.0 / math.sqrt(self.layer2.out_features)
    ...
    def forward(self, dino_latent: torch.Tensor=None, edge_latent: torch.Tensor=None):
        ...
        dino_latent = self.layer1(dino_latent)
        dino_latent = self.layer2(dino_latent)
