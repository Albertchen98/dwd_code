...
def process_frames_with_seg_guided_filter():
    ...
    for i, frame_path in enumerate(frame_files):
        ...
        actor = actors[i % num_gpus]
        results.append(actor.process_single_frame.remote(frame_path, seg_path, highres_scale))
    
    # ============ 收集结果 ============
    all_features = []
    for _ in tqdm(range(len(results)), desc="Processing frames"):
        done, results = ray.wait(results)
        all_features.append(ray.get(done[0]))
    
    ray.shutdown()
    
    # ============ 拼接并保存 ============
    # 按原始顺序排序（因为 ray.wait 可能乱序）
