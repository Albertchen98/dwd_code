...
def read_and_resize_dino_video_input(
    input_video_path: str,
    input_dino_path: str,
    num_total_frames: int = NUM_MAX_FRAMES,
    interpolation: int = cv2.INTER_AREA,
    ...
    input_video = resize_video(input_video, h, w, interpolation=interpolation)  # BCTHW, range [0, 255]
    input_video = torch.from_numpy(input_video[0])  # CTHW, range [0, 255]
    dino_tensor = load_file(input_dino_path)
    dino_tensor = dino_tensor["dino"][:num_total_frames,5:]  # t (h w) c
    dino_tensor = einops.rearrange(dino_tensor, "t (h w) c -> c t h w ", h=h//16, w=w//16)
    return dino_tensor, input_video, fps, aspect_ratio, original_hw
...
def read_and_process_control_input(
    ...
    s3_credential_path: str | None = None,
    use_prep_anyup_feature: bool = False,
    prep_anyup_feature_channel: int | None = None,
    anyup_factor: int | None = None,
):
    """
    ...
    for modality in hint_key:
        ...
        # assert control_path is not None, f"Control path for {modality} is missing"  
        control_key = f"control_input_{modality}"

        if control_path and os.path.exists(control_path):
            if "dino" in modality and use_prep_anyup_feature:
                # TODO 这里修改成加载预处理好dino anyup 特征的逻辑
                log.info(f"Loading anyup control input from {control_path}")
                control_attr = torch.load(control_path, map_location='cpu')
                control_attr = control_attr.squeeze(0) # B C T H W -> C T H W
                if anyup_factor is not None and anyup_factor != 8:
                    target_h = 44 * anyup_factor
                    target_w = 80 * anyup_factor
                    control_attr = torch.nn.functional.interpolate(
                        control_attr, 
                        size=(target_h, target_w), 
                        mode='bilinear', 
                        align_corners=False
                    )
                    log.info(f"Resized anyup feature to {target_h}x{target_w}")
                if prep_anyup_feature_channel is not None:
                    control_attr = control_attr[:prep_anyup_feature_channel]  # select first N channels
                    log.info(f"Selected first {prep_anyup_feature_channel} channels for anyup feature")
            ...
            # if "dino" in modality: 
            #     control_attr, _, fps, _, _ = read_and_resize_dino_video_input(
            #         video_path,
            #         control_path,
            #         resolution=resolution,
            #         interpolation=config["interpolation"],
            #         s3_credential_path=s3_credential_path,
            #     )
            #     log.info("control_input_dino loaded")
            # else:
...
def parse_control_input_file_paths(
    ...
    input_control_folder_dino=None,
    ...
    input_control_folder_dino_mask=None,
    input_control_folder_inpaint_mask=None,
    video_file=None,
    use_prep_anyup_feature=None,
) -> dict[str, str]:
    """
    ...
    control_modalities = {
        ...
        "dino": input_control_folder_dino,
        ...
        "dino_mask": input_control_folder_dino_mask,
    ...
    for modality, folder in control_modalities.items():
        if folder is not None:
            if modality == "dino" and use_prep_anyup_feature:
                control_video_paths[modality] = os.path.join(folder, control_filename.replace(".mp4", ".pt"))
            else:
    ...
    return parse_control_input_single_file_paths(
        ...
        input_control_video_path_dino=control_video_paths["dino"],
        ...
        input_control_video_path_dino_mask=control_video_paths["dino_mask"],
...
def parse_control_input_single_file_paths(
    ...
    input_control_video_path_dino=None,
    ...
    input_control_video_path_dino_mask=None,
    ...
    control_modalities = {
        ...
        "dino": input_control_video_path_dino,
        ...
        "dino_mask": input_control_video_path_dino_mask,
