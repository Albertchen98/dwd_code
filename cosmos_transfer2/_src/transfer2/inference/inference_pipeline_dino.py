    ...
    def generate_img2world(
        ...
        if self.text_encoder_class == "T5":
            ...
            if isinstance(prompt, list):
                text_embeddings_multi = self.model.text_encoder.compute_text_embeddings_online(
                    {"ai_caption": prompt, "images": None}, input_caption_key="ai_caption"
                )
                text_embeddings = [text_embeddings_multi[[idx]] for idx in range(len(text_embeddings_multi))]
            else:
        ...
        log.info("Loading  controlinputs...")
        control_input_dict = read_and_process_control_input(
            ...
            use_prep_anyup_feature=use_prep_anyup_feature,
            prep_anyup_feature_channel=prep_anyup_feature_channel,
            anyup_factor=anyup_factor
        )
        
        # 根据首尾帧裁剪视频和控制输入
        start_frame = 0 if start_frame is None else start_frame
        end_frame = input_frames.shape[1] if end_frame is None else end_frame
        input_frames = input_frames[:, start_frame:end_frame]
        for control_key in control_input_dict:
            control_input_dict[control_key] = control_input_dict[control_key][:, start_frame:end_frame]
        # 根据首尾帧裁剪文本嵌入
        if isinstance(text_embeddings, list):
            start_text_id = start_frame // 92
            end_text_id = end_frame // 92
            print(f"start_text_id: {start_text_id}, end_text_id: {end_text_id}")
            if reverse:
                text_embeddings = text_embeddings[start_text_id:end_text_id][::-1]
            else:
                text_embeddings = text_embeddings[start_text_id:end_text_id]
        if reverse:
            input_frames = torch.flip(input_frames, dims=[1])
            for control_key in control_input_dict:
                control_input_dict[control_key] = torch.flip(control_input_dict[control_key], dims=[1])
        ...
        for chunk_id in range(num_chunks):
            ...
            for k, v in control_input_dict.items():
                cur_control_input = v[:, chunk_start_frame:chunk_end_frame].to("cuda")
            ...
            # Accumulate control inputs for each chunk for visualization
            # for key in hint_key:
            #     control_input = data_batch["control_input_" + key]
            #     if "dino" in key:
            #         control_input = visualize_dino_pca_video(control_input)
                
            #     if f"control_input_{key}_mask" in data_batch:
            #         try:
            #             control_input = (control_input + 1) / 2 * data_batch[f"control_input_{key}_mask"] * 2 - 1
            #         except:
            #             control_input = (control_input + 1) / 2 * data_batch[f"control_input_{key}_mask"].to("cuda") * 2 - 1
        ...
        if device_rank == 0:
            ...
            if reverse:
                full_video = torch.flip(full_video, dims=[2])
