...
from cosmos_transfer2._src.transfer2.inference.inference_pipeline_dino import ControlVideo2WorldInference
...
def print_resource_memory_usage():
    """打印进程的最大常驻集大小 (Max RSS)"""
    # ru_maxrss 是常驻集大小（Resident Set Size）的最大值
    # 在 Linux 上通常以 KB 为单位，macOS/BSD 上以字节为单位
    # 这里假设是 KB
    mem_kb = resource.getrusage(resource.RUSAGE_SELF).ru_maxrss
    mem_gb = mem_kb / (1024 * 1024)
    print("--- 资源库内存占用 (Max RSS) ---")
    print(f"最大常驻集大小: {mem_gb:.2f} GB")
    print("--------------------------------")
...
def process_single_video(
    ...
    if video_exists:
        log.info(color_message(f"Video already exists at {save_path}. Overwriteing...", "yellow"))
        # return
    ...
    print(f"device_rank is {device_rank}")
    # Run model inference
    output_video, control_video_dict, fps, original_hw = inference_pipeline.generate_img2world(**inference_kwargs)

    if device_rank == 0:
        log.info(f"saving video to {save_path}.mp4")
        print_resource_memory_usage()
        output_video = (1.0 + output_video[0]) / 2
        for key in control_video_dict:
            control_video_dict[key] = (1.0 + control_video_dict[key][0]) / 2
            save_img_or_video(control_video_dict[key], f"{save_path}_control_{key}", fps=fps)
            log.info(f"{key} control video saved to {save_path}_control_{key}.mp4")
...
def main() -> None:
    ...
    registered_exp_name="vid2vid_2B_control_720p_t24_dinocontrol_layer4_cr1_embedding_rectified_flow_train_keyframe"
    exp_override_opts=["model.config.text_encoder_config.compute_online=True",
                       "model.config.tokenizer.compile_encode=False",
                       "model.config.net.sample_dino_key_frame=False",
                       "model.config.sample_dino_key_frame=False"
                   ]
