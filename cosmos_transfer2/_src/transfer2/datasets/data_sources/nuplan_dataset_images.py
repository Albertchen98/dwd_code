...
"""
class BilateralFilter(v2.Transform):
    def __init__(
        self,
        d: int = 7,
    ...
    def _transform(self, img: torch.Tensor) -> torch.Tensor:
        ...
        # back to Tensor[C, H, W], float
        img_out = torch.from_numpy(img_np).permute(2, 0, 1).float() / 255.0
        return img_out

    def forward(self, img):
        # v2 官方推荐：在 forward 里处理 structured input
...
class Dataset(Dataset):
    def __init__(
        ...
        Args:
            ...
            dino_augmentations: optiions ["color_jitter", "blurriness"]
        Returns dict with:
        ...
        assert os.path.exists(self.dataset_dir), f"dataset_dir {self.dataset_dir} does not exisv2."
        ...
        if "color_jitter" in dino_augmentations:
            ...
            self.color_jitter = v2.Identity()
            
        if "blurriness" in dino_augmentations:
            self.blurriness = BilateralFilter(d=15, sigma_color=100, sigma_space=100, p=0.2)
            
        else:
            self.blurriness = v2.Identity()
        ...
        self.dino_aug = v2.Compose([v2.ToDtype(torch.float32, scale=True), self.color_jitter, self.blurriness, self.normalize])
        
        self.preprocess = v2.Compose([ToTensorVideo(), ResizePreprocess(tuple([h, w]))])
    ...
    def __getitem__(self, index) -> dict | Any:
        ...
        t5_embedding_path = os.path.join(self.t5_dir, f"{stem}.pickle")
        ...
        n_tokens = text_embeddings.shape[0]
        if n_tokens < self.len_t5:
            text_embeddings = np.concatenate(
                [text_embeddings, np.zeros((self.len_t5 - n_tokens, self.t5_dim), dtype=np.float32)], axis=0
            )
        t5_text_mask = torch.zeros(self.len_t5, dtype=torch.int64)
        t5_text_mask[:n_tokens] = 1
        
        data["t5_text_embeddings"] = torch.from_numpy(text_embeddings)
        ...
        for key in self.hint_keys:
            ...
            if "dino" in key:
                # print(video.shape)
                data["control_input_dino"] = self.dino_aug(video.squeeze(1))[:,None]
                # import torchvision
                # torchvision.utils.save_image(data["control_input_dino"].clip(0,1).squeeze(1), "test.png")
        return data

if __name__ == "__main__":
    dataset = Dataset(
        dataset_dir="/home/ma-user/work/z00595504/dataset/nuplan/videos",
        num_video_frames=1,
        resolution="720",
        hint_keys="dino",
        dino_augmentations=["color_jitter","blurriness"]
    )
    ...
    for idx in range(10000):
        ...
        log.info(
            (
                ...
                f"{data['control_input_dino'].max()=}\n"
                f"{data['control_input_dino'].min()=}\n"
