...
class AddControlInputDino(Augmentor):
    """
    Add control input to the data dictionary. control input are expanded to 3-channels
    steps to add new items: modify this file, configs/conditioner.py, conditioner.py
    """
    ...
    def __init__(
        self,
        input_keys: list,
        output_keys: Optional[list] = ["control_input_dino"],
        args: Optional[dict] = None,
        **kwargs,
    ) -> None:
        super().__init__(input_keys, output_keys, args)
    ...
    def __call__(self, data_dict: dict) -> dict:
        assert "control_input_dino" in data_dict
        # if "control_input_dino" in data_dict:
            # already processed
        return data_dict
...
CTRL_HINT_KEYS = {
    ...
    "control_input_dino": AddControlInputDino,
}
