...
class ControlDinoEdge2ImageConfig(Text2WorldModelConfig):
...
class ControlDinoEdge2ImageModel(DiffusionModel):
    """
    ImaginaireModel instance of the VACE-styled controlnet for training.
    """

    def __init__(self, config: ControlDinoEdge2ImageConfig, *args, **kwargs):
    ...
    def get_control_latent(self, latent_state: Tensor, hint_key, control_input: Tensor, control_input_mask: Tensor) -> Tensor:
        ...
        if "dino" not in hint_key:
            ...
            # dino control
            control_input = control_input.to(**self.tensor_kwargs) / 255.0
            
            control_input = rearrange(control_input, "B C T H W -> (B T) C H W")
            control_input = F.normalize(
                                control_input, 
                                mean=IMAGENET_DEFAULT_MEAN, 
                                std=IMAGENET_DEFAULT_STD
                            )
            control_input = rearrange(control_input, "(B T) C H W -> B C T H W", B=B)
            control_input = self.dinov3_encoder(control_input)
            B, C, T, H, W = control_input.shape
            control_input = rearrange(control_input, "b c t h w -> (b t) c h w ")
            control_input = torch.nn.functional.interpolate(control_input, scale_factor=2, mode='bilinear', align_corners=False)
