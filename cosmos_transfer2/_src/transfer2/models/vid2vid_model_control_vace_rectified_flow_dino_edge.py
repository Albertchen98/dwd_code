...
class ControlDinoEdge2WorldRectifiedFlowConfig(Video2WorldModelRectifiedFlowConfig):
...
class ControlDinoEdge2WorldModelRectifiedFlow(Video2WorldModelRectifiedFlow):
    """
    ImaginaireModel instance of the VACE-styled controlnet for training.
    """

    def __init__(self, config: ControlDinoEdge2WorldRectifiedFlowConfig, *args, **kwargs):
        self.is_new_training = True
        self.copy_weight_strategy = config.copy_weight_strategy
        self.hint_keys = ["control_input_edge", "control_input_dino"]
    ...
    def get_data_and_condition(
        ...
        for hi, hint_key in enumerate(self.hint_keys):
            ...
            latent_control_input += self.get_control_latent(latent_state, hint_key, control_input, control_input_mask)
    ...
    def get_control_latent(self, latent_state: Tensor, hint_key, control_input: Tensor, control_input_mask: Tensor) -> Tensor:
        latent_control_input = []
        if "dino" not in hint_key:
            latent_control_input.append(self.encode(control_input).contiguous().to(**self.tensor_kwargs))
        else:
            # dino control
            control_input = control_input[:,:,::4].to(**self.tensor_kwargs)
            B, C, T, H, W = control_input.shape
            control_input = rearrange(control_input, "b c t h w -> (b t) c h w ")
            control_input = F.interpolate(control_input, scale_factor=2, mode='bilinear', align_corners=False)
            control_input = rearrange(control_input, "(b t) c h w -> b c t h w", b=B, t=T)
            latent_control_input.append(control_input)
