...
class ControlDinoPrep2WorldRectifiedFlowConfig(Video2WorldModelRectifiedFlowConfig):
    ...
    feature_mode: str = "offline"  # offline PCA cache or online frozen DINOv3
    dinov3_encoder: LazyDict = None  # DINO V3 encoder configuration


class ControlDinoPrep2WorldModelRectifiedFlow(Video2WorldModelRectifiedFlow):
    """
    ImaginaireModel instance of the VACE-styled controlnet for training.
    """

    def __init__(self, config: ControlDinoPrep2WorldRectifiedFlowConfig, *args, **kwargs):
        self.is_new_training = True
        self.copy_weight_strategy = config.copy_weight_strategy
        self.hint_keys = ["control_input_dino"]
        super().__init__(config, *args, **kwargs)
        if config.feature_mode not in ("offline", "online"):
            raise ValueError("feature_mode must be offline or online")
        if config.feature_mode == "online":
            if config.dinov3_encoder is None:
                raise ValueError("Online training requires a dinov3_encoder config")
            self.dinov3_encoder = lazy_instantiate(config.dinov3_encoder)
            self.dinov3_encoder.requires_grad_(False)
    ...
    def get_control_latent(self, latent_state: Tensor, control_input: Tensor, control_input_mask: Tensor) -> Tensor:
        # Both modes reach the temporal adapter as [B, PCA channels, T, H/16, W/16].
        if self.config.feature_mode == "online":
            # Dataset supplies ImageNet-normalized RGB, so use_processor stays False.
            self.dinov3_encoder.train(self.training)
            self.dinov3_encoder.model.eval()
            control_input = self.dinov3_encoder(control_input.to(**self.tensor_kwargs))
        latent_control_input = []
        if self.config.sample_dino_key_frame:
            latent_control_input.append(control_input[:, :, ::4].to(**self.tensor_kwargs))
        else:
            # 93 RGB frames -> 96 padded feature frames -> 24 temporal tokens.
